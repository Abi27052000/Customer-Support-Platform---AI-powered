...
interface RefDataSidebarProps {
  ...
  selectedKeyName: string; // 🔹 added for active highlight
  selectedSubMenu: string; // 🔹 added for active highlight
}
...
export const RefDataSidebar: React.FC<RefDataSidebarProps> = ({
  ...
  const filtered = useMemo(() => {
    ...
    return menuItems.filter((it) => {
      const name =
          (it as any).title || (it as any).name || (it as any).label || it.keyName || "";
      return name.toLowerCase().includes(q);
    });
  }, [menuItems, query]);

  // 🔹 Automatically expand parent of selected submenu
  React.useEffect(() => {
    setOpenMenu(selectedKeyName);
  }, [selectedKeyName]);
            ...
            {filtered.map((item) => (
                <li key={item.keyName} className="group">
                  <RefDataSidebarItem
                      ...
                      onSelect={onSelect}
                      activeSubMenu={selectedSubMenu} // 🔹 controlled highlight
                      isActive={selectedKeyName === item.keyName} // 🔹 controlled highlight
                  />
                </li>
            ))}
          </ul>
