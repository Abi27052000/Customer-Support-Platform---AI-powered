...
export const OrganizationRegistration: React.FC = () => {
  ...
  const validateStep = () => {
    ...
    if (activeStep === 1) {
      if (!form.adminUsername.trim())
        e.adminUsername = "Username (email) required";
  ...
  const submit = async () => {
    ...
    setApiError(null);
    try {
      const res = await fetch('/api/admin/register-org', {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        ...
        setApiError(data.message || 'Failed to register organization');
      }
    } catch (err) {
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-lg p-8">
                ...
                <div className="space-y-6">
                  ...
                  {apiError && (
                    <div className="bg-red-50 text-red-600 p-4 rounded-lg text-sm">{apiError}</div>
                  )}

                  {/* Actions */}
                  <div className="mt-4 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <div className="flex-1 sm:flex-auto">
                      <Button variant="ghost" onClick={back} disabled={activeStep === 0 || loading}>← Back</Button>
                    </div>
